...
**Votes after Hidden Gem declaration**         
...
wss <- function(k) {
  kmeans(scale(kvcs_reduced), k, nstart = 10 )$tot.withinss
}
...
kvcs_reduced = as.data.frame(scale(kvcs_reduced))

set.seed(20)
clusters <- kmeans(kvcs_reduced, 4)

kvcs_reduced$clusterNumber <- clusters$cluster

showCluster <- function(clusterNo) {
  
  cluster <- kvcs_reduced %>%
    filter(clusterNumber ==  clusterNo)
  
  sort(colSums(cluster),decreasing = TRUE)%>%
    as.data.frame() %>%
    rownames_to_column() %>%
    filter(rowname != "clusterNumber") %>%
    head(4) %>%
    select(rowname) %>%
    kable()
...
plotCluster <- function(clusterNo) {
  ...
  sort(colSums(cluster),decreasing = TRUE)%>%
    ...
    rownames_to_column() %>%
    filter(rowname != "clusterNumber") %>%
    mutate(rowname = reorder(rowname,.)) %>%
    
    
    ggplot(aes(x = rowname,y = .,label = rowname, fill = rowname)) +
    geom_col(alpha = 0.9, show.legend = FALSE) +
    geom_text(aes(rowname, 0.001), hjust=0, vjust=.5, size = 4, colour = 'black') +
    labs(x = NULL, 
         y = 'Cumulative Value') +
    coord_flip() + 
